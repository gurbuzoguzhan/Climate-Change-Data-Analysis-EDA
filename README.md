...
#These discoveries will help us comprehend the magnitude of climate change and its potential ramifications for our ecosystem. 
...
install.packages('ggthemes')    # for more plotting options
install.packages('gridExtra')   # for arranging plots
...
# Calculate the overall seasonal average and add it to the dataset
temperature_v2 <- temperature_v2 %>%
  mutate(seasonal_average = round(apply(select(., winter, spring, summer, autumn), 1, mean, na.rm = TRUE), 2)
```
